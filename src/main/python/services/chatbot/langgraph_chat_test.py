...
def test_sons_of_jabob():
  ...
  assert "one" in query("How many daughters?")
...
def test_remembers_context():
  ...
  assert "three" in query("How many wives did he have?")
  assert len(agent.messages) == 14
  assert "Terah" in query("What was his father's name?")
  assert len(agent.messages) == 16
  assert "Haran" in query("What was the name of his brother that died?")
  assert len(agent.messages) == 18
  assert "Nahor" in query("What was the name of his other brother?")
  assert len(agent.messages) == 20
  assert "What does Genesis 2:3 say?" in agent.messages[0]['content']
  assert "Elohim blessed the seventh day" in agent.messages[1]['content']
  assert "twelve" in query("How many sons did that brother have?")
  assert len(agent.messages) == 20
