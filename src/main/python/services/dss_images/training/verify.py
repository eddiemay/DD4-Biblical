...
def verify(verify_request):
			...
			for bf in [None, 7]:
				for blur in [None, 'median', 'gaussian']:
					...
					for blur_size in [3, 5] if blur is not None else [None]:
						for threshold_type in [None, cv2.THRESH_BINARY,
																	 cv2.THRESH_BINARY_INV, cv2.THRESH_TRUNC]:
							for threshold in threshold_values if threshold_type is not None else [
								None]:
								evaluated.append(
	...
	result['best'] = best
	print(
		f'{name}, {best["parameters"]["model"]}, {best["name"]}, {best["ld"]}, {best["percent"]}%')
	...
	if display:
		...
		d = pytesseract.image_to_data(best_image, lang=model,
		...
		for i in range(6):
			...
			plt.subplot(3, 2, i + 1), plt.imshow(
					process_image(img, top[ii]['parameters'])[0])
...
def to_verify_request(name, img_file, txt, model=None,
...
def output_column_stats(model=None, use_best=False,
	...
	with open('verify.csv', 'w') as csv:
		...
		for result in results:
			...
			output(csv, result["name"],
						 [result["best"]["percent"], result["best"]["name"],
							overall_best["percent"], overall_best["preprocessor_name"],
							overall_best['parameters']['model']])
		output(csv, '', [])
		...
		output(csv, 'Median', [np.round(np.median(percents), decimals),
													 np.round(np.median(overall_bests), decimals)])
		output(csv, 'Mode', [stats.mode(np.round(percents / 5) * 5).mode,
												 stats.mode(np.round(overall_bests / 5) * 5).mode])
		std = np.round(np.std(percents), decimals)
		overall_std = np.round(np.std(overall_bests), decimals)
		output(csv, 'Std', [std, overall_std])
		output(csv, 'Z-Low', [np.round(mean - std * 1.96, decimals),
													np.round(overall_mean - overall_std * 1.96,
																	 decimals)])
		output(csv, 'Z-High', [np.round(mean + std * 1.96, decimals),
													 np.round(overall_mean + overall_std * 1.96,
																		decimals)])
	...
	if extras:
		for fragment in [50, 48, 16, 7, 1, 54]:
			...
			for model in ['Hebrew_Font_Embedding_Label_22',
										'Hebrew_Font_Embedding_Label_22_22K_nosp',
										'Hebrew_Font_Embedding_Label_22_22K',
										'Hebrew_Font_Embedding_Label_19_sp',
										'Hebrew_Font_Embedding_Label_19_gblur',
										'Hebrew_Font_Embedding_Label_19_otsu',
										'Hebrew_Font_Embedding_Label_19_16K',
										'Hebrew_Font_Embedding_Label_14',
										'Hebrew_Font_Embedding_Label_17', BEST_MODEL]:
				verify(
					to_isa_verify_request(fragment, model, use_best=True, display=False))
		# Jody 937-630-2748
		...
		for img_file in image_files:
			for model in ['Hebrew_Font_Label_14', 'Hebrew_Font_Embedding_Label_14',
										BEST_MODEL]:
				verify(to_verify_request(img_file, img_file, txt, model, use_best=True,
