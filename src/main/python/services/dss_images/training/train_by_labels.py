...
def process(sample):
	...
	cleared = cv2.inpaint(row_img, mask, 3, cv2.INPAINT_TELEA)
...
def display(sample):
	print(sample['text'])
	print(sample['boxes'])
	output_base = FILE_BASE_NAME.format(sample['fragment'], sample['res'],
																			sample['srow'], sample['erow'])
...
if __name__ == '__main__':
	...
	display(process(
			{'fragment': 'isaiah-column-44', 'srow': 25, 'erow': 25, 'res': 10}))
	display(
		process({'fragment': 'isaiah-column-44', 'srow': 25, 'erow': 27, 'res': 9}))
	display(
		process({'fragment': 'isaiah-column-44', 'srow': 1, 'erow': 7, 'res': 8}))
	display(
		process({'fragment': 'isaiah-column-44', 'srow': 1, 'erow': 28, 'res': 8}))
	...
	command = ['make', 'training', 'MODEL_NAME=label',
						 f'START_MODEL={start_model}',
						 'TESSDATA=../tessdata_best', f'MAX_ITERATIONS={ITERATIONS * 1024}']
	print(' '.join(command))
	subprocess.run(command)
	command = ['cp', 'data/label.traineddata',
						 f'/opt/homebrew/share/tessdata/dabar.cloud/{MODEL_NAME}.traineddata']
	print(' '.join(command))
	subprocess.run(command)
